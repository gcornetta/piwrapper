...
module.exports.controller = function (req, res) {
  ...
  dashboardPage.currentPanelRoute = '/dashboard/control/milling/shopbot'
...
var _validate = function (req, res) {
  ...
  if (errors.length === undefined) { return [] } else { return errors }
}
  ...
  form.on('field', function (field, value) {
    ...
    switch (field) {
      ...
      case 'yz' : req.body.yz = value
        break
      case 'isInches' : req.body.isInches = value
        break
        // Position
      ...
      case 'type' : req.body.type = value
        break
      case 'cut_speed' : req.body.cut_speed = value
        break
      case 'plunge_speed' : req.body.plunge_speed = value
        break
      case 'jog_speed' : req.body.jog_speed = value
        break
      case 'jog_height' : req.body.jog_height = value
        break
      case 'spindle_speed' : req.body.spindle_speed = value
        break
    }
    for (var f in fields) {
      fifoData[f] = fields[f]
    ...
    sync.fiber(function () {
      ...
      if (errors.length > 0) {
        ...
        fifo.push(fifoData, 'local', function (err, job) {
          if (err) {
            logger.error('@controllers.shopbot_milling: ' + err.err)
...
module.exports.process = function (req, res) {
  ...
  logger.info('@dashboard.processsssssssssss: the process selected is ' + req.query.process)
    console.log("yiiiiiiiiiiiiiii")
  if (req.query.process === 'pcb') {
    res.render('partials/process/milling-machines/shopbot/cut', dashboardPage)
  }
  if (req.query.process === 'wax') {
    res.render('partials/process/milling-machines/shopbot/wax', dashboardPage)
  }
}
