...
var ADS1x15 = function (ic, address, i2c_dev) {

  ic = (typeof ic !== 'undefined' ? ic : IC_ADS1015);                   // default to ads1015...
  address = (typeof address !== 'undefined' ? address : 0x48);          // Address pin tied to ground gives us 1001000 (or 0x48)
  i2c_dev = (typeof i2c_dev !== 'undefined' ? i2c_dev : '/dev/i2c-1');  // default to pi 2b/3...
  ...
  this.wire = new i2c(address, { device : i2c_dev } );   // Raspberry Pi2 uses I2c address 1... (change to '/dev/i2c-1' for old Pi 1)
...
// othewise it returns the value for 250sps. This saves a lot of if/elif/else code!
  ...
  self.wire.writeBytes(ADS1015_REG_POINTER_CONFIG, bytes, function(err) {
    if(err) {
      console.log("ADS1x15: Write error. " + err.toString());
      callback(err);
    }
    ...
    setTimeout(function() {
     // Read the conversion results
      self.wire.readBytes(ADS1015_REG_POINTER_CONVERT, 2, function(err, res) {
        if(err) {
          console.log("ADS1x15: Read error. " + err.toString());
          callback(err);
        }
        if (self.ic == IC_ADS1015) {
      ...
      else
      {
        ...
        if (val > 0x7FFF)
        {
          var data = (val - 0xFFFF) * pga / 32768.0;
        }
        else
        {
          var data = ( (res[0] << 8) | (res[1]) ) * pga / 32768.0;
        }
        callback(null, data);
      }
    });
...
ADS1x15.prototype.readADCDifferential = function(chP, chN, pga, sps, callback) {
  ...
  self.wire.writeBytes(ADS1015_REG_POINTER_CONFIG, bytes, function(err) {
    if(err) {
      callback("ADS1x15: Write error. " + err.toString());
    }
  });
  // Wait for the ADC conversion to complete
  // The minimum delay depends on the sps: delay >= 1/sps
...
ADS1x15.prototype.getLastConversionResults = function(callback) {
  ...
  this.wire.readBytes(ADS1015_REG_POINTER_CONVERT, 2, function(err, res) {
    if (this.ic == IC_ADS1015) {
      ...
      if (val > 0x7FFF)
      {
        var data =  (val - 0xFFFF) * this.pga / 32768.0;
      }
      else
      {
        var data =  ( (res[0] << 8) | (res[1]) ) * this.pga / 32768.0;
      }
    }
  });
}
...
// Starts the comparator mode on the specified channel
...
ADS1x15.prototype.startSingleEndedComparator = function(channel, thresholdHigh, thresholdLow, pga, sps, activeLow, traditionalMode, latching, numReadings, callback) {
  ...
  if (this.ic == IC_ADS1015) {
    var thresholdHighWORD = int(thresholdHigh*(2048.0/pga));
  }
  else {
    var thresholdHighWORD = int(thresholdHigh*(32767.0/pga));
  }
  ...
  if (this.ic == IC_ADS1015) {
    var thresholdLowWORD = int(thresholdLow*(2048.0/pga));
  }
  else {
    var thresholdLowWORD = int(thresholdLow*(32767.0/pga));
  }
