...
//var socketioClient = require('socket.io-client')('http://pigateway.local:3000');
...
module.exports.ws = function(server, eventEmitter){
    ...
    io.sockets.on("connection", function(socket) {
        ...
        socket.on('removeJob', function(jobId){
            if ((fifo.getJobById(jobId))&&((fifo.getJobById(jobId).status !== "running")&&(fifo.getJobById(jobId).status !== "cancelled"))){
            ...
            Machine.checkIfMachineConfigured(function(err, machine){
                if (err){
                    ...
                    printFunctions.printStringJob(machine, str, function(err){
                        if (err){
                            console.log(err);
                            socket.emit('directCommandResult', 'error');
                        }else{
