...
var socketioClient = require('socket.io-client')('http://piwrapper.local');
    ...
    io.sockets.on("connection", function(socket) {
        ...
        socket.on('removeJob', function(jobId){
            if (fifo.getJobById(jobId).status !== "running"){
                ...
                socket.emit('errorAlert', {err: "You can't remove active job"});
            }
        });
        ...
        socket.on('directCommand', function(str){
            ...
            Machine.checkIfMachineConfigured(function(err, machine){
                if (err){
                    ...
                    printFunctions.printStringJob(machine, str, function(){
