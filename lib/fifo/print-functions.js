  ...
  printing = true;
      calculateJob(that, printingJob, path, machineLib, machineVar, function(str){
        fs.writeFile(printingJobGlobal.jobId+"JobFile", str, function(err, data) {
            ...
            printer.printFromFile(machineVar.name, printingJobGlobal.jobId+"JobFile", function(job){
  ...
  if (!printing){
    ...
    fs.writeFile(printingJobGlobal.jobId+"JobFile", str, function(err, data) {
        if(err) {
            ...
            var cmd = 'sudo python mod_print.py /dev/usb/lp0 ";" "' + printingJobGlobal.jobId+'JobFile"';
            var child = exec(cmd, function(error, stdout, stderr) {
               fs.unlink(printingJobGlobal.jobId+"JobFile", function(err) {if (err) {logger.error('@print-functions.printStringJob: '+ err)}});
               if (error == null) {
                  logger.debug('@print-functions.printStringJob: '+ "jobFinished");
                  printing = false;
                  callback();
               } else {
                  logger.error('@print-functions.printStringJob: '+ stderr);
                  printing = false;
                  callback(stderr);
               }
            });
