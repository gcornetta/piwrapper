...
functions.calculateAndPrint2 = function(that, printingJob, path, machineLib, machineVar){
    calculateJob(that, printingJob, path, machineLib, machineVar, function(str){
      fs.writeFile(printingJobGlobal.jobId+"epilog", str, function(err, data) {
          if(err) {
              logger.error('@print-functions.calculateAndPrint2: '+ err);
              repushJob(that);
          }
          var cmd = 'sudo python mod_print.py /dev/usb/lp0 ";" "' + printingJobGlobal.jobId+"epilog" + '"';
          console.log(cmd)
          var child = exec(cmd, function(error, stdout, stderr) {
             console.log(stdout)
             fs.unlink(printingJobGlobal.jobId+"epilog", function(err) {
                if (err) throw err
             });
             if (error == null) {
                logger.debug('@print-functions.calculateAndPrint2: '+ "jobFinished");
                that.removeJob(printingJobGlobal.jobId, printingJobGlobal.caller, function(){
                    that.executeNextJob();
                });
             } else {
                logger.error('@print-functions.calculateAndPrint2: '+ stderr);
                repushJob(that);
             }
          });
      });
...
functions.calculateAndPrint3 = function(that, printingJob, path, machineLib, machineVar){
  calculateJob(that, printingJob, path, machineLib, machineVar, function(str){
      fs.writeFile(printingJobGlobal.jobId+"epilog", str, function(err, data) {
          if(err) {
              logger.error('@print-functions.calculateAndPrint3: '+ err);
              repushJob(that);
          }
          printer2.printFile({filename: printingJobGlobal.jobId+"epilog",
              printer: process.env[3] // printer name, if missing then will print to default printer
              , success:function(jobID){
                  		logger.debug('@print-functions.calculateAndPrint3: '+ "jobFinished");
                          that.removeJob(printingJobGlobal.jobId, printingJobGlobal.caller, function(){
                              that.executeNextJob();
                          });
                  	}
