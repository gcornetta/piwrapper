module.exports.newPriorityFifo = function (){
  ...
  jobFifo.push = function(job, callback){
    if (!this.userMap[job.userId]){
      if (this.priorityQueue.length > this.MAXUSERS-1){
    ...
    if (this.userMap[job.userId].jobs.length > this.MAXUSERJOBS-1){
    ...
    this.winston.info('@fifo.push: new job pushed into the FIFO');
    this.winston.info('@fifo.push: job details ' + JSON.stringify(job));
    var that = this;
  ...
  jobFifo.update = function(job, callback){
    if ((job)&&(this.jobMap[job.jobId])){
      ...
      this.machine.updateJob(job, function (err, product, num){
        ...
        that.winston.info('@fifo.update: new job updated into the FIFO');
        that.winston.info('@fifo.update: job details ' + JSON.stringify(job));
    ...
    if (this.jobMap[jobId]){
      ...
      this.machine.removeJobById(jobId, function(err){
        ...
        if (that.userMap[job.userId]){
          ...
          that.winston.info('@fifo.remove: job remove from FIFO');
          that.winston.info('@fifo.remove: job details ' + JSON.stringify(job));
          callback(err, job);
        }else{
          callback({err: "Job user doesn't exist"}, null);
        }
      });
    }else{
      callback({err: "Job doesn't exist"}, null);
