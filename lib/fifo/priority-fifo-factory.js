module.exports.newPriorityFifo = function (maxUsers){
  ...
  var jobFifo = {};
    ...
    jobFifo.socketioClient = require('socket.io-client')('http://pigateway.local:3000');
    ...
    if (!this.jobMap[job.jobId]){
        ...
        this.machine.addMachineJob(job, function (err, product, num){
            ...
            if (that.userMap[job.userId]){
                ...
                callback({err: "Job already removed"}, null);
            }
        });
    }else{
    ...
    if ((job)&&(this.jobMap[job.jobId])){
      ...
      this.machine.updateJob(job, function (err, product, num){
        ...
        if (this.socketioClient){
            this.socketioClient.emit('updateJob', JSON.stringify(job));
        }
  ...
  jobFifo.updatePriority = function(newPriority, userId){
    if (this.userMap[userId]){
      this.userMap[userId].priority = newPriority;
        if (this.socketioClient){
            this.socketioClient.emit('updateUserPriority', userId, newPriority);
        }
    ...
    if (this.jobMap[jobId]){
      ...
      this.machine.removeJobById(jobId, function(err){
        ...
        if (that.userMap[job.userId]){
          ...
          if (this.socketioClient){
            this.socketioClient.emit('deleteJob', jobId);
          }
          ...
          this.fs.unlink(job.jobPath, function(err) {if (err) {logger.error('@fifo.remove file error: '+ err)}});
          callback(err, job);
        }else{
  ...
  jobFifo.lastJobCompleted = function(){
    ...
    if (user){
      if (user.jobs.length > 0){
            ...
            if (this.socketioClient){
                this.socketioClient.emit('deleteJob', job.jobId);
            }
  ...
  jobFifo.resetPriorityIndex = function(){
    ...
    for (var i in this.userMap){
        ...
        if (this.socketioClient){
            this.socketioClient.emit('updateUserPriority', i, this.userMap[i].priority);
        }
