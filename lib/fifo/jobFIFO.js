...
var fs = require('fs');
var pathFunction = require('../fablab/lib/path2D');
var path = new pathFunction();
var io;
var fifoLocal;
var fifoAPI;
var nextLocal = true;
...
if ((fifo.getUsersLength() > MAXUSERS-1)&&(fifo.getUserJobsLength(job.userId) === 0)){
  ...
  callback(null, job);
  //this.acceptJob(job.jobId, callback);
}
}
...
function calculateAndPrintJob(that){
  ...
  .on('parsed', function() {
    ...
    path.pathImage2D(printingJob, function(p, dpi){
      printingJob.dpi = dpi;
