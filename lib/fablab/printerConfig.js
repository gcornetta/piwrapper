...
module.exports.addPrinter = function(printer, callback) {
  exec('lpadmin -p '+printer.Name+' -E -v '+printer.DeviceURI, (error, stdout, stderr) => {
    if (error) {
      callback(error);
      return;
    }
    if (!stderr) {
      exec('lpoptions -d '+printer.Name, (error, stdout, stderr) => {
          if (error) {
            callback(error);
            return;
          }
...
module.exports.printFromString = function(text, callback) {
