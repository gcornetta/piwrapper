...
TrotecLaser.prototype.cmdString = function (callback) {
  ...
  if (this.startCorner === 'bottom left') {
    ...
    xoffset = xorg + 1000 * (this.ox - dx) / umPerInc
    yoffset = yorg + 1000 * (this.oy - dy) / umPerInc
  } else if (this.startCorner === 'top left') {
    xoffset = xorg + 1000 * this.ox / umPerInc
    yoffset = yorg + 1000 * this.oy / umPerInc
  } else { // top right
    xoffset = xorg + 1000 * (this.ox - dx) / umPerInc
    yoffset = yorg + 1000 * this.oy / umPerInc
  }
  ...
  for (var seg = 0; seg < this.path.length; ++seg) {
    ...
    var y = yoffset + scale * (ny - this.path[seg][0][1])
    if (x < 0) x = 0
    if (y < 0) y = 0
