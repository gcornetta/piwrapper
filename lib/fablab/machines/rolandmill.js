   ...
   RolandMill.prototype.rolandCmdString = function (callback) {

      var dx = toMillimeters (this.width / this.dpi);
      var nx = this.width;
      
      if (this.model == 'mdx 15' || this.model == 'mdx 20') {
         var rmlUnit = 40.0;
      } else // models mdx 40 and srm 20
         var rmlUnit = 100.0;
      }
      
      var ijog = Math.floor(rmlUnit * jog)
      var scale = rmlUnit * dx / (nx - 1);
      var xoffset = rmlUnit*x0;
      var yoffset = rmlUnit*y0;
      var zoffset = rmlUnit*z0;
      ...
      var xh = rml_unit * this.xhome;
      var yh = rml_unit * this.yhome;
      var zh = rml_unit * this.zhome;
