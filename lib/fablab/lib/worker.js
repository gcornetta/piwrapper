...
function followEdges (img, error) {
  ...
  for (var row = 1; row < (img.height - 1); ++row) {
    for (var i = 1; i < (img.width - 1); ++i) {
      if (((row + 2) % 2) === 0) { col = i } else { col = img.width - i - 1 }
      follow_edges(row, col)
    }
  }
  return path
...
fn['calculatePath2D'] = function (args) {
  process.send(['prompt', 'calculatePath2D'])
  var img = args.img  // process_img img is a Uint8ClampedArray object
  // var output_img = args[2]
  // var outImg = JSON.parse(JSON.stringify(args.img));
  var outImg = args.img2
  process.send(['prompt', 'outImg'])
  var threshold = args.threshold
  var offset = Number(args.offset) // number
  ...
  // threshold
  //
  process.send(['prompt', 'threshold'])
  calcThreshold(img, threshold)
  //
  // find distances
  //
  process.send(['prompt', 'calculate distances'])
  findDistances(img)
  //
  // offset
  //
  ...
  while (true) {
    n += 1
    process.send(['prompt', 'offset ' + n + '/' + offset])
    ...
    if (offsetPath.length > 0) {
      ...
      for (var i = 0; i < segOrder.length; ++i) { segOrder[i] = n }
    ...
    // return path;
    if ((n === offset) || (offsetPath.length === 0)) { break }
...
fn['imageOffsetZ'] = function (args) {
  ...
  // set height
  //
  process.send(['prompt', 'set height'])
  ...
  if (xz) {
    ...
    for (var row = ir; row <= (img.height - ir); row += id) {
      process.send(['prompt', 'row ' + row + '/' + img.height])
      ...
      // add to path
      //
      for (var pt = 0; pt < newpath.length; ++pt) { path[path.length - 1][path[path.length - 1].length] = newpath[pt] }
      // process.send(['path', path])
      sign = -sign
    }
  }
  ...
  if (yz) {
    ...
    for (var col = ir; col <= (img.width - ir); col += id) {
      process.send(['prompt', 'column ' + col + '/' + img.width])
...
function calcOffset (distances, distance, img) {
  ...
  for (var row = 0; row < img.height; ++row) {
    for (var col = 0; col < img.width; ++col) {
      if (view.getUint32((distances.height - 1 - row) * distances.width * 4 + col * 4, false) <= distance) { img.set(row, col, STATE, INTERIOR) } else { img.set(row, col, STATE, EMPTY) }
...
function sortWeightedPath (path, pathOrder, mergeDistance, orderWeight, sequenceWeight) {
  ...
  if (sequenceWeight > 0) {
    ...
    x0 = path[path.length - 1][path[path.length - 1].length - 1][X]
    y0 = path[path.length - 1][path[path.length - 1].length - 1][Y]
  ...
  while (path.length > 0) {
    ...
    if (Math.sqrt(dmin) <= mergeDistance) { newpath[newpath.length - 1] = newpath[newpath.length - 1].concat(path[segmin]) } else { newpath[newpath.length] = path[segmin] }
...
function calcThreshold (img, threshold) {
  ...
  for (var row = 0; row < img.height; ++row) {
    for (var col = 0; col < img.width; ++col) {
      var intensity = (img.get(row, col, 0) + (img.get(row, col, 1) << 8) + (img.get(row, col, B) << 16)) / imax
      if (intensity > threshold) { img.set(row, col, STATE, INTERIOR) } else { img.set(row, col, STATE, EMPTY) }
      img.set(row, col, DIRECTION, NONE)
    }
  }
}
