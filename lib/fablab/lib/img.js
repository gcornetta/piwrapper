...
var imgRead = function (img, callback) {
  fs.createReadStream(img)
      ...
      .on('parsed', function () {
        callback({data: this.data, width: this.width, height: this.height}) // eslint-disable-line
      })
      .on('error', function () {
        callback(null)
      })
...
function modImageGet (row, col, element) {
  return this.data[(this.height - 1 - row) * this.width * 4 + col * 4 + element]
}

function modImageSet (row, col, element, value) {
      ...
      while (y + r < img.height) {
        ...
        if ((i * (r - dr)) > rmin) {
          ...
          while (radius > 0) {
            ...
            if ((dr > 0)/* && spot_fill */) { radius -= 2 * dr } else break
          }
        }
        x += delta
      }
